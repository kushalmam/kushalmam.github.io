...
const projectHtml = (project: (typeof projects)[number]) => `
  <article>
    <h3>${escape(project.name)}</h3>
    <p>${escape(project.detail)}</p>
    <p>${escape(project.contribution)}</p>
    <p>${escape(project.stack)}</p>
    <p><strong>${escape(project.evidenceValue + " " + project.evidenceUnit.trim())}</strong> · ${escape(project.context)}</p>
    ${project.caseStudy.map((part) => `<p><strong>${escape(part.title)}.</strong> ${escape(part.text)}</p>`).join("")}
    ${project.sources.map((source) => `<p><a href="${escape(source.href)}">${escape(source.label)} ↗</a></p>`).join("")}
  </article>`;
...
export function staticFallback(base: string) {
  return `<main class="static-fallback">
    ...
    <p>I build the systems that move, shape, and serve data at scale.</p>
    <section><h2>Currently at Spotify</h2><p>Improving personalization data pipelines as a Data Engineer (Emerging Talent).</p></section>
    <section><h2>Selected system</h2>${projectHtml(projects[0])}</section>
    ...
    <section><h2>More projects</h2>${projects.slice(1).map(projectHtml).join("")}</section>
