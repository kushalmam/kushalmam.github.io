...
export function signalProgress(points: Point[], scroll: number, viewport: number, mainTop: number, about: number, maxScroll: number) {
  ...
  const progress = readingLine();
  const arrivalStart = Math.max(handoff, maxScroll - Math.min(260, viewport * .3));
  if (scroll <= arrivalStart || maxScroll <= arrivalStart) return progress;
  const t = Math.max(0, Math.min(1, (scroll - arrivalStart) / (maxScroll - arrivalStart)));
  const ease = t * t * (3 - 2 * t);
  return progress + (1 - progress) * ease;
}
