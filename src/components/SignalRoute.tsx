...
export default function SignalRoute() {
  ...
  const [layout, setLayout] = useState({ d: "", heroD: "", width: 1, height: 1, mainTop: 0, about: 0 });
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const name = document.querySelector("h1 > span")!.getBoundingClientRect();
      const startX = name.right - rect.left + 12;
      const startY = name.top - rect.top + name.height * .68;
      let d = mobile
      ...
      setLayout({ d, heroD, width, mainTop: rect.top + window.scrollY, about, height: main.offsetHeight });
    };
  ...
  useEffect(() => {
    ...
    import("../scene/createSignalScene").then(({ createSignalScene }) => {
      ...
      try {
        scene = createSignalScene({ canvas: canvas.current, points, width: layout.width, mainTop: layout.mainTop, about: layout.about, onLost: () => { setReady(false); scene?.dispose(); scene = undefined; } });
        paint();
        setReady(true);
